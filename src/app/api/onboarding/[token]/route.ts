...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  // Wrapped in try/catch so any unexpected failure still returns JSON —
  // an unhandled exception here would otherwise surface as an HTML error
  // page, which breaks res.json() on the client and leaves the onboarding
  // form stuck on "Submitting…" forever.
  try {
    const { token } = await params;
    ...
    const { email, password, sex, ...rest } = body as {
      email?: string;
      password?: string;
      sex?: string;
      [key: string]: unknown;
    ...
    if (client.invite_expires_at && new Date(client.invite_expires_at) < new Date()) {
      return NextResponse.json({ error: "This invite has expired — ask your coach for a new link" }, { status: 410 });
    }

    const form = parsed.data;
    const unitPreference = form.unitPreference ?? "METRIC";
    ...
    if (updateError) {
      // The auth user was already created at this point — roll it back so a
      // retry doesn't hit "email already exists" on an orphaned account.
    ...
    return NextResponse.json(
      { error: "Something went wrong completing your profile. Please try again or contact your coach." },
      { status: 500 }
    );
  }
}
